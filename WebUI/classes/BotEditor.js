import { InputDeviceKeys } from "./Constants.js";
...
class BotEditor {
	...
	bindMouseEvents() {
		...
		document.body.addEventListener("mousedown", (event) => {
			...
			switch (parent.dataset.action) {
				...
				case "restore":
					[].map.call(getClosest(event.target, "ui-view").querySelectorAll("ui-entry"), (entry) => {
				...
				case "trace_save":
					let index = document.querySelector(
						'input[type="number"][name="trace_index"]'
					);
					WebUI.Call(
						"DispatchEventLocal",
						"BotEditor",
				...
				default:
					...
					if (parent.dataset.action.startsWith("submit")) {
						let form = getClosest(event.target, "ui-view").querySelector('[data-type="form"]');
	...
	openSettings(data) {
		...
		let container = document.querySelector(
			'ui-view[data-name="settings"] figure'
		);
	...
	updateTraceIndex(index) {
		document.querySelector('input[type="number"][name="trace_index"]').value =
			index;
	}
	...
	updateTraceWaypointsDistance(distance) {
		...
		document.querySelector('ui-value[data-name="distance"]').innerHTML =
			distance;
	}
	...
	setOperationControls(data) {
		...
		if (json.Other) {
			let otherKeysElement = document.querySelector(
				'ui-entry[data-grid="Other"]'
			);

...
def get_to_root() -> None:
    ...
    try:
        new_cwd = "/".join(
            cwd_splitted[
                : cwd_splitted.index(
                    fun_bots_name_branch[-2],
                    len(cwd_splitted)
                    - cwd_splitted[::-1].index(fun_bots_name_branch[-2])
                    - 1,
                )
                + 1
            ]
        ...
        new_cwd = "/".join(
            cwd_splitted[
                : cwd_splitted.index(
                    fun_bots_name_branch[-1],
                    len(cwd_splitted)
                    - cwd_splitted[::-1].index(fun_bots_name_branch[-1])
                    - 1,
                )
                + 1
            ]
            ...
            if (
                ...
                try:
                    ...
                    check_grammar = requests.post(
                        "https://api.languagetool.org/v2/check", data=data, timeout=10
                    ).json()

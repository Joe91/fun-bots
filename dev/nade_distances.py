...
def Distance(angle):
    return Speed * Speed * math.sin(2* angle) / (2 * Drop) + Speed * math.cos(angle)/Drop * math.sqrt(Speed*Speed*math.sin(angle)*math.sin(angle) + 2 * Drop * Height)
...
with open ("temp.txt", "w") as outfile:
    for key, value in outTable.items():
        ...
        if FirtstLine:
            FirtstLine = False
            startOfLine = "if"
        outfile.write(startOfLine+ " self._DistanceToPlayer > "+ str(key) + " then s_GrenadePitch = " + str(value) + "\n")
    outfile.write("end")
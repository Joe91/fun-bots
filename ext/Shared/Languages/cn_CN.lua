...
Language:add(code, "Vehicles move while shooting", "车辆在射击时移动")
Language:add(code, "Vehicles like tanks do not stop for shooting", "坦克等车辆不会停下来射击")
Language:add(code, "Speed factor vehicle attack", "速度因素车辆攻击")
Language:add(code, "Reduces the movement speed while attacking in vehicles. 1 = normal, 0 = standing", "在车辆中攻击时降低移动速度。 1 = 正常，0 = 站立")
Language:add(code, "Damage Factor Vehicles", "损坏因素车辆")
Language:add(code, "Original Damage from bots in vehicles gets multiplied by this", "车辆中机器人的原始伤害乘以此值")
Language:add(code, "Vehicle Aim Worsening", "车辆目标恶化")
Language:add(code, "Make bots in vehicles aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "让车辆中的机器人瞄准更差：难度：0 = 无偏移（困难），1 或什至更大 = 更多摇摆（简单）")
Language:add(code, "Vehicle Air Aim Worsening", "车辆空中瞄准恶化")
Language:add(code, "See VehicleAimWorsening, only for Air-Vehicles", "请参阅 VehicleAimWorsening，仅适用于飞行器")

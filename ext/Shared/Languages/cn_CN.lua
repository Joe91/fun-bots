...
Language:add(code, "Damage Factor Vehicles", "损坏载具的因素")
...
Language:add(code, "Target distance waypoint air vehicles", "目标距离航路点飞行器")
Language:add(code, "The distance the bots have to reach to continue with the next Waypoint on air vehicles", "机器人必须到达的距离才能继续飞行器上的下一个航点")
Language:add(code, "Additional reaciton-time of bots", "机器人的额外反应时间")
Language:add(code, "Additional delay for bots, dependant of skill (might also be 0)", "机器人的额外延迟，取决于技能（也可能为 0）")
Language:add(code, "Point of Interst", "兴趣点")
Language:add(code, "Sniper-Spot", "狙击点")
Language:add(code, "POI", "兴趣点")
Language:add(code, "Beacon", "灯塔")

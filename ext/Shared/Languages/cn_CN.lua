...
Language:add(code, "Randomize Helth of Zombies", "僵尸的生命值随机化")
Language:add(code, "zombie-helth differs from bot to bot", "僵尸的生命值因不同BOT而异")
Language:add(code, "Randomize Attack-Speed of Zombies", "僵尸的攻击速度随机")
Language:add(code, "zombie-speed differs from bot to bot", "僵尸速度因不同BOT而异")
Language:add(code, "Randomize Damage that zombies deal", "僵尸的伤害随机化")
Language:add(code, "zombie-damage differs from bot to bot", "僵尸伤害因不同BOT而异")
Language:add(code, "Randomize Jump-Speeds of zombies", "僵尸的跳跃速度随机化")
Language:add(code, "zombie-high-jumps differs from bot to bot", "僵尸跳跃高度因不同BOT而异")
Language:add(code, "Zombies prone", "僵尸俯伏")
Language:add(code, "Zombies can prone when walking around", "僵尸行动时可以俯卧")
Language:add(code, "Zombies crouch", "僵尸下蹲")
Language:add(code, "Zombies can crouch when walking around", "僵尸行动时可以蹲下")
Language:add(code, "Zombies walk", "僵尸行走")
Language:add(code, "Zombies can walk when walking around", "僵尸行动时可以步行")
Language:add(code, "Zombies sprint", "僵尸冲刺")
Language:add(code, "Zombies can sprint when walking around", "僵尸行动时可以冲刺")
Language:add(code, "Bot Max Health at spawn", "Bot在重生时的最大生命值")
Language:add(code, "Max health of bot at spawn(default 100.0)", "生成BOT的最大生命值（默认100.0）")
Language:add(code, "Bot Min Health at spawn", "Bot Min Health at spawn")
Language:add(code, "Min health of bot at spawn (default 100.0)", "生成BOT的最小生命值（默认100.0）")
Language:add(code, "Bot Min Health at spawn", "生成BOT最小生命值")
Language:add(code, "Damage multiplier for shooting bots in the head", "射击BOT头部的伤害系数")
Language:add(code, "Modifies the speed while attacking. 1 = normal", "修改攻击时的速度. 1 = 正常")
Language:add(code, "Min Speed factor attack", "最小攻击速度系数")
Language:add(code, "Modifies the minimal speed while attacking. 1 = normal", "修改攻击时的最小速度. 1 = 正常")
Language:add(code, "Min High Jump Speed", "最小跳跃速度")
Language:add(code, "Min Speed the bots jump with on high-jumps", "BOT跳跃时的最小速度")
Language:add(code, "Max High Jump Speed", "最大跳跃速度")
Language:add(code, "Max Speed the bots jump with on high-jumps", "BOT跳跃时的最大速度")
Language:add(code, "Max waves", "最大波数")
Language:add(code, "Total amount of waves needed to win. 0 = infinite", "获胜所需的游戏波数. 0 = 无限")
Language:add(code, "Player Lives", "玩家存活次数")
...
Language:add(code, "Additional Zombies per wave", "每波额外添加的僵尸")
Language:add(code, "Zombies that are added in each new wave", "每一波新加入的僵尸")
Language:add(code, "Additional Max Health per wave", "每波额外的最大生命值")
...
Language:add(code, "Zombies deal more damage each wave", "僵尸每一波造成更多伤害")
...
Language:add(code, "Zombies get more speed each wave", "僵尸在每一波中获得更快的速度")
Language:add(code, "Decrease spawn distance per wave", "减少每波的生成距离")
Language:add(code, "Decreases the spawn distance each wave. This can help sell the effect of a continuous wave when the server slot limit is reached", "减少每波的生成距离.当服务器人数已满时增进打僵尸的效率")
Language:add(code, "Zombies alive for next wave", "触发下一波次的僵尸数量")
Language:add(code, "New wave is triggered when this number of zombies is reached", "达到此僵尸数量时会触发新的波次")
Language:add(code, "Time between waves", "波与波之间的时间")
...
Language:add(code, "Remaining Bots Get Killed before a new wave starts", "剩余的BOT在新一波开始前被杀死")

...
Language:add(code, "Damage Factor Assault", "突击兵专属武器伤害倍率")
Language:add(code, "Original Damage from bots gets multiplied by this", "BOT原伤害乘以此")
Language:add(code, "Damage Factor Carabine", "工程兵专属武器伤害倍率")
Language:add(code, "Original Damage from bots gets multiplied by this", "BOT原伤害乘以此")
Language:add(code, "Damage Factor LMG", "支援兵专属武器伤害倍率")
Language:add(code, "Original Damage from bots gets multiplied by this", "BOT原伤害乘以此")
Language:add(code, "Damage Factor PDW", "冲锋枪伤害倍率")
Language:add(code, "Original Damage from bots gets multiplied by this", "BOT原伤害乘以此")
Language:add(code, "Damage Factor Sniper", "侦察兵专属武器伤害倍率")
Language:add(code, "Original Damage from bots gets multiplied by this", "BOT原伤害乘以此")
...
Language:add(code, "Max Distance a normal soldier shoots back if Hit", "普通士兵被击中后反击的最大距离")
Language:add(code, "Meters until bots (not sniper) shoot back if hit", "BOT（非狙击手）被击中后反击的最大距离")
Language:add(code, "Max Distance a sniper soldier shoots back if Hit", "狙击手被击中后反击的最大距离")
Language:add(code, "Meters until snipers shoot back if hit", "狙击手BOT被击中后反击的最大距离")
...
Language:add(code, "Bots can kill themselves", "BOT会自杀")
Language:add(code, "Bot takes fall damage or explosion-damage from own frags", "BOT将受到摔落和爆炸造成的伤害")
...
Language:add(code, "Bots revive other players", "BOT会救治其余玩家")
...
Language:add(code, "Bots deploy ammo and medkits", "BOT将会部署弹药或者医疗箱")
...
Language:add(code, "Degrees of FOV of AA - Vehicles", "AA防空车的FOV视野")
Language:add(code, "Max Raycast Distance for Vehicles", "防空载具的最大射击距离")
Language:add(code, "Meters bots in Vehicles start shooting at players", "防空载具的BOT射击玩家的最远距离")
Language:add(code, "Max Shoot-Distance No Anti Air", "非防空载具的最大射击距离")
Language:add(code, "Meters bots in vehicle (no Anti-Air) starts shooting at players", "非防空载具的BOT射击玩家的最远距离")
...
Language:add(code, "Speed factor", "移动速度倍率")
Language:add(code, "Reduces the movement speed. 1 = normal, 0 = standing", "降低移动速度. 1 = 常规,0 = 不动")
Language:add(code, "Speed factor attack", "攻击时的移动速度倍率")
Language:add(code, "Reduces the movement speed while attacking. 1 = normal, 0 = standing", "攻击时降低移动速度. 1 = 常规,0 = 不动")
...
Language:add(code, "Bot min time shoot at player", "BOT向玩家射击的最短时间")
...
Language:add(code, "Bot fire mode duration in vehicle", "BOT在载具内的开火时长")
...
Language:add(code, "Speed factor vehicle attack", "载具攻击时的移动速度倍率")
Language:add(code, "Reduces the movement speed while attacking in vehicles. 1 = normal, 0 = standing", "在载具攻击时降低移动速度。 1 = 常规，0 = 不动")
Language:add(code, "Damage Factor Vehicles", "BOT对载具造成的伤害倍率")
...
Language:add(code, "Bot Team Names", "BOT队伍名称")
Language:add(code, "If the bots shall have their names based on their team", "即便BOT可以拥有它们自己的名字，但必须基于它们的队伍名称")
Language:add(code, "The minimum time a bot shoots at one player", "BOT向一名玩家射击的最短时间")
Language:add(code, "Explore", "探索")
Language:add(code, "Draw Parsed Spawns", "绘制已解析的重生点")
Language:add(code, "Draw the parsed spawn-points with parsed links", "绘制已解析的重生点及其关联链接")
Language:add(code, "AA Aim Worsening", "BOT使用AA的瞄准精度")
Language:add(code, "Make bots in AA aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "BOT操控AA的瞄准精度：难度：0 = 无偏移（困难），1 或者更大 = 更多偏移（简单）")
Language:add(code, "Gunship Aim Worsening", "使用AC-130的瞄准精度")
Language:add(code, "Make bots in Gunships aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "BOT操控AC-130的瞄准精度：难度：0 = 无偏移（困难），1 或者更大 = 更多偏移（简单）")
Language:add(code, "Chopper Aim Worsening", "使用直升机的瞄准精度")
Language:add(code, "Make bots in Choppers aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "BOT使用直升机的瞄准精度：难度：0 = 无偏移（困难），1 或者更大 = 更多偏移（简单）")
Language:add(code, "Vehicle Plane Aim Worsening", "固定翼的瞄准精度")
Language:add(code, "See VehicleAimWorsening, only for Planes", "载具可见范围内的瞄准精度，仅适用于喷气机")
Language:add(code, "Max Shoot Distance Air Missiles", "对空导弹的最远射击距离")
...
Language:add(code, "Spawn in Transport helicopters and AMTRACs", "运输直升机和Amtrac的产卵")
Language:add(code, "Bots can spawn in Transport helicopters and AMTRACs", "机器人可以在运输直升机和Amtrac中产生")

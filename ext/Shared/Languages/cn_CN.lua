...
Language:add(code, "Draw Parsed Spawns", "画出解析的产卵")
Language:add(code, "Draw the parsed spawn-points with parsed links", "用解析链接绘制解析的产卵点")
Language:add(code, "AA Aim Worsening", "AA目标恶化")
Language:add(code, "Make bots in AA aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "使AA的机器人更糟：难度：0 =无偏移（硬），1甚至更大")
Language:add(code, "Gunship Aim Worsening", "武装业的目标恶化")
Language:add(code, "Make bots in Gunships aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "使武装行动的机器人更糟：难度：0 =无偏移（硬），1甚至更大=更多的摇摆（容易）")
Language:add(code, "Chopper Aim Worsening", "切碎的目标恶化")
Language:add(code, "Make bots in Choppers aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "使斩波器中的机器人的目标更糟：难度：0 =无偏移（硬），1甚至更大=更多的摇摆（容易）")
Language:add(code, "Vehicle Plane Aim Worsening", "车飞机的目标恶化")
Language:add(code, "See VehicleAimWorsening, only for Planes", "参见车辆持有，仅适用于飞机")
Language:add(code, "Max Shoot Distance Air Missiles", "最大射击距离空气导弹")
Language:add(code, "Meters before bots will start shooting with stingers", "机器人之前的仪表将开始与刺痛者射击")

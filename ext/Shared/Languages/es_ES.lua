...
Language:add(code, "Draw Parsed Spawns", "Dibujar desovadores analizados")
Language:add(code, "Draw the parsed spawn-points with parsed links", "Dibuja los puntos de desove analizados con enlaces analizados")
Language:add(code, "AA Aim Worsening", "AA AIME EN LA ENGRANACIÓN")
Language:add(code, "Make bots in AA aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Haga que los bots en AA apunten peor: para la dificultad: 0 = sin compensación (dura), 1 o incluso mayor = más balance (fácil)")
Language:add(code, "Gunship Aim Worsening", "Gunship Aim empeoramiento")
Language:add(code, "Make bots in Gunships aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Hacer que los bots en cañoneros apunten peor: para dificultad: 0 = sin desplazamiento (duro), 1 o incluso mayor = más balance (fácil)")
Language:add(code, "Chopper Aim Worsening", "AIME CHOCEPER empeorando")
Language:add(code, "Make bots in Choppers aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Hacer que los bots en los helicópteros apunten peor: para la dificultad: 0 = sin desplazamiento (duro), 1 o incluso mayor = más infiltrado (fácil)")
Language:add(code, "Vehicle Plane Aim Worsening", "Año de avión del vehículo empeoramiento")
...
Language:add(code, "Keep a Vehicle Seat For Player", "Mantenga un asiento de vehículo para el jugador")
Language:add(code, "To keep a vehicle seat for the player always or not (default true)", "Para mantener un asiento de vehículo para el jugador siempre o no (verdadero verdadero)")
Language:add(code, "Spawn in Transport helicopters and AMTRACs", "Engendrar en helicópteros de transporte y amtracs")
Language:add(code, "Bots can spawn in Transport helicopters and AMTRACs", "Los bots pueden aparecer en helicópteros de transporte y amtracs")
Language:add(code, "Follow Me", "Sígueme")
Language:add(code, "Stop Following", "Dejar de seguir")

...
Language:add(code, "Draw Parsed Spawns", "解析されたスポーンを描きます")
Language:add(code, "Draw the parsed spawn-points with parsed links", "解析されたリンクで解析されたスポーン点を描画します")
Language:add(code, "AA Aim Worsening", "AAの目的は悪化します")
Language:add(code, "Make bots in AA aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "AAのボットを悪化させる：難易度：0 =オフセットなし（ハード）、1またはさらに大きい=より多くの揺れ（簡単）")
Language:add(code, "Gunship Aim Worsening", "ガンシップは悪化を目指しています")
Language:add(code, "Make bots in Gunships aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "ガンシップのボットを悪化させる：難易度：0 =オフセットなし（硬）、1、またはさらに大きい=より多くの揺れ（簡単）")
Language:add(code, "Chopper Aim Worsening", "チョッパーは悪化します")
Language:add(code, "Make bots in Choppers aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "チョッパーのボットをより悪化させる：難易度：0 =オフセットなし（ハード）、1、またはさらに大きい=より多くの揺れ（簡単）")
Language:add(code, "Vehicle Plane Aim Worsening", "車両の飛行機の目的が悪化します")
Language:add(code, "See VehicleAimWorsening, only for Planes", "飛行機のみの車両を参照してください")
Language:add(code, "Max Shoot Distance Air Missiles", "マックスシュート距離空気ミサイル")

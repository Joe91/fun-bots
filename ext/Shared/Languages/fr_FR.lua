...
Language:add(code, "Reduce playerteam in balanced_teams or fixed_number mode", "Réduire l'équipe de joueurs en mode balancing_teams ou fixed_number")
...
Language:add(code, "Bots can kill themself", "Les robots peuvent se tuer")
Language:add(code, "Bot takes falldamage or explosion-damage from own frags", "Le bot subit des dégâts de chute ou d'explosion de ses propres frags")
...
Language:add(code, "Min time bots move sidewards or staight before switching (in sec)", "Temps minimum pendant lequel les bots se déplacent latéralement ou droit avant de changer (en secondes)")
...
Language:add(code, "Use a random weapon out of the weaponset", "Utiliser une arme aléatoire hors de l'ensemble d'armes")
...
Language:add(code, "Draw Spawn Points", "Dessiner des points d'apparition")
Language:add(code, "Range of Spawnpoints", "Gamme de points d'apparition")
Language:add(code, "Set how far away spawnpoints are visible (meters)", "Définir à quelle distance les spawnpoints sont visibles (mètres)")
Language:add(code, "Debug Selection Raytraces", "Débogage des lancers de rayons de sélection")
Language:add(code, "Shows the trace line and search area from Commo Rose selection", "Affiche la ligne de tracé et la zone de recherche de la sélection Commo Rose")
...
Language:add(code, "Bots without sniper aim for the head. More an experimental config", "Les robots sans tireur d'élite visent la tête. Plus une configuration expérimentale")
Language:add(code, "Bots with Sniper aim for head", "Bots avec Sniper viser la tête")
Language:add(code, "Bots with sniper aim for the head. More an experimental config", "Les robots avec un tireur d'élite visent la tête. Plus une configuration expérimentale")
Language:add(code, "Bots with Support LMGs aim for head", "Les robots avec support LMG visent la tête")
Language:add(code, "Bots with support LMGs aim for the head. More an experimental config", "Les bots avec support LMG visent la tête. Plus une configuration expérimentale")
...
Language:add(code, "Speed facator", "Facteur de vitesse")
Language:add(code, "Reduces the movementspeed. 1 = normal, 0 = standing.", "Réduit la vitesse de déplacement. 1 = normal, 0 = debout.")
Language:add(code, "Speed facator attack", "Attaque de facteur de vitesse")
Language:add(code, "Reduces the movementspeed while attacking. 1 = normal, 0 = standing.", "Réduit la vitesse de déplacement pendant l'attaque. 1 = normal, 0 = debout.")
...
Language:add(code, "Delay for first shot. If too small, there will be great spread in first cycle because its not compensated yet.", "Délai pour le premier coup. S'il est trop petit, il y aura une grande propagation au premier cycle car il n'est pas encore compensé.")
...
Language:add(code, "in Degrees. Rotaion-Movement per second.", "en Degrés. Rotaion-Mouvement par seconde.")
Language:add(code, "Target distance waypoint", "Point de cheminement à distance cible")
Language:add(code, "Distance the bots have to reach to continue with the next Waypoint", "Distance que les bots doivent atteindre pour continuer avec le prochain Waypoint")
...
Language:add(code, "de_DE as sample (default is english, when language file doesnt exists)", "de_DE comme exemple (la valeur par défaut est l'anglais, lorsque le fichier de langue n'existe pas)")

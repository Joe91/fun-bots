...
Language:add(code, "Damage Factor Vehicles", "Véhicules à facteur de dégâts")
Language:add(code, "Original Damage from bots in vehicles gets multiplied by this", "Les dégâts d'origine des bots dans les véhicules sont multipliés par cela")
Language:add(code, "Vehicle Aim Worsening", "Détérioration de la visée du véhicule")
Language:add(code, "Make bots in vehicles aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Faire en sorte que les bots dans les véhicules visent moins bien : pour la difficulté : 0 = pas de décalage (difficile), 1 ou même plus = plus de balancement (facile)")
Language:add(code, "Target distance waypoint air vehicles", "Véhicules aériens à distance cible et point de cheminement")
Language:add(code, "The distance the bots have to reach to continue with the next Waypoint on air vehicles", "La distance que les robots doivent atteindre pour continuer avec le prochain Waypoint sur les véhicules aériens")
Language:add(code, "Additional reaciton-time of bots", "Temps de réaction supplémentaire des robots")
Language:add(code, "Additional delay for bots, dependant of skill (might also be 0)", "Délai supplémentaire pour les robots, en fonction des compétences (peut également être de 0)")
Language:add(code, "Point of Interst", "Point d'intérêt")
Language:add(code, "POI", "POI")
Language:add(code, "Beacon", "Balise")
Language:add(code, "Use jets", "Utiliser des jets")
Language:add(code, "Bots can use jets", "Les robots peuvent utiliser des jets")
Language:add(code, "Bots Revive Bots", "Les robots font revivre les robots")
Language:add(code, "Bots revive other Bots", "Les robots font revivre d'autres robots")
Language:add(code, "Defend objectives", "Défendre les objectifs")
Language:add(code, "Bots will stay on captured objectives and defend them", "Les robots resteront sur les objectifs capturés et les défendront")
Language:add(code, "Enable vehicle paradrop", "Activer le parachutage du véhicule")
Language:add(code, "Bots can spawn on vehicles inside C-130 gunship", "Les robots peuvent apparaître sur les véhicules à l'intérieur du navire de combat C-130")
Language:add(code, "Bot Team Names", "Noms des équipes de robots")
Language:add(code, "If the bots shall have their names based on their team", "Si les robots doivent avoir leurs noms basés sur leur équipe")
Language:add(code, "The minimum time a bot shoots at one player", "Le temps minimum pendant lequel un bot tire sur un joueur")
Language:add(code, "Explore", "Explorer")
Language:add(code, "Draw Parsed Spawns", "Dessiner des fraises analysées")
Language:add(code, "Draw the parsed spawn-points with parsed links", "Dessinez les points d'apparition analysés avec des liens analysés")
Language:add(code, "AA Aim Worsening", "AA AIM aggraver")
Language:add(code, "Make bots in AA aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Rendre les bots dans un objectif AA pire: pour la difficulté: 0 = pas de décalage (dur), 1 ou même plus grand = plus d'influence (facile)")
Language:add(code, "Gunship Aim Worsening", "Vise au navire de combat aggravant")
Language:add(code, "Make bots in Gunships aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Rendre les robots en navires de combat pour le bon, pour la difficulté: 0 = pas de décalage (dur), 1 ou même plus grand = plus de balancement (facile)")
Language:add(code, "Chopper Aim Worsening", "Hopper Bid aggraver")
Language:add(code, "Make bots in Choppers aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Rendre les bots dans les hachoirs pire: pour la difficulté: 0 = pas de décalage (dur), 1 ou même plus grand = plus de balancement (facile)")
Language:add(code, "Vehicle Plane Aim Worsening", "Bualiment d'avion du véhicule aggravant")
Language:add(code, "See VehicleAimWorsening, only for Planes", "Voir VehicleAiMworsening, uniquement pour les avions")
Language:add(code, "Max Shoot Distance Air Missiles", "Missiles d'air de distance Max Shoot")

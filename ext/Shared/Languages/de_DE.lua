...
Language:add(code, "Make bots aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy).", "Lassen Sie Bots schlechter zielen: für Schwierigkeit: 0 = kein Versatz (schwer), 1 oder noch größer = mehr Schwanken (einfach).")
...
Language:add(code, "See BotWorseningSkill - only for BOTs using sniper bolt-action rifles.", "Siehe BotWorseningSkill – nur für BOTs, die Scharfschützen-Repetierbüchsen verwenden.")
...
Language:add(code, "Reduce playerteam in balanced_teams or fixed_number mode", "Reduzieren Sie das Spielerteam im Modus „balanced_teams“ oder „fixed_number“.")
...
Language:add(code, "Meters before bots will start shooting at players", "Meter bevor Bots anfangen, auf Spieler zu schießen")
...
Language:add(code, "Meters until snipers shoot back if hit", "Meter, bis Scharfschützen zurückschießen, wenn sie getroffen werden")
...
Language:add(code, "Bot takes falldamage or explosion-damage from own frags", "Bot erleidet Fallschaden oder Explosionsschaden durch eigene Frags")
...
Language:add(code, "Min time bots move sidewards or staight before switching (in sec)", "Min. Zeit, in der sich Bots seitwärts oder geradeaus bewegen, bevor sie wechseln (in Sek.)")
...
Language:add(code, "Meters bots in Vehicles start shooting at players", "Meter-Bots in Fahrzeugen fangen an, auf Spieler zu schießen")
Language:add(code, "Max Shoot-Distance No Anti Air", "Max. Schussweite Kein Anti Air")
Language:add(code, "Meters bots in vehicle (no Anti-Air) starts shooting at players", "Meter-Bots im Fahrzeug (kein Luftabwehr) fangen an, auf Spieler zu schießen")
...
Language:add(code, "If false choppers only attack without gunner on board", "Wenn falsche Hubschrauber nur ohne Richtschützen an Bord angreifen")
...
Language:add(code, "Set how far away spawnpoints are visible (meters)", "Legen Sie fest, wie weit entfernte Spawnpunkte sichtbar sind (Meter)")
...
Language:add(code, "Bots without sniper aim for the head. More an experimental config", "Bots ohne Scharfschützen zielen auf den Kopf. Eher eine experimentelle Konfiguration")
Language:add(code, "Bots with Sniper aim for head", "Bots mit Sniper zielen auf den Kopf")
Language:add(code, "Bots with sniper aim for the head. More an experimental config", "Bots mit Scharfschützen zielen auf den Kopf. Eher eine experimentelle Konfiguration")
Language:add(code, "Bots with Support LMGs aim for head", "Bots mit Unterstützungs-LMGs zielen auf den Kopf")
Language:add(code, "Bots with support LMGs aim for the head. More an experimental config", "Bots mit Unterstützungs-LMGs zielen auf den Kopf. Eher eine experimentelle Konfiguration")
...
Language:add(code, "Reduces the movementspeed. 1 = normal, 0 = standing.", "Reduziert die Bewegungsgeschwindigkeit. 1 = normal, 0 = stehend.")
Language:add(code, "Speed facator attack", "Geschwindigkeitsfaktor-Angriff")
Language:add(code, "Reduces the movementspeed while attacking. 1 = normal, 0 = standing.", "Verringert die Bewegungsgeschwindigkeit beim Angriff. 1 = normal, 0 = stehend.")
...
Language:add(code, "Delay for first shot. If too small, there will be great spread in first cycle because its not compensated yet.", "Verzögerung für den ersten Schuss. Wenn es zu klein ist, wird es im ersten Zyklus eine große Streuung geben, da es noch nicht kompensiert ist.")
...
Language:add(code, "Distance the bots have to reach to continue with the next Waypoint", "Entfernung, die die Bots erreichen müssen, um mit dem nächsten Wegpunkt fortzufahren")
...
Language:add(code, "de_DE as sample (default is english, when language file doesnt exists)", "de_DE als Beispiel (Standard ist Englisch, wenn keine Sprachdatei vorhanden ist)")

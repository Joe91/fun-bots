...
Language:add(code, "Defend objectives", "Ziele verteidigen")
Language:add(code, "Bots will stay on captured objectives and defend them", "Bots bleiben auf eroberten Zielen und verteidigen sie")
Language:add(code, "Enable vehicle paradrop", "Fahrzeug-Fallschirmabwurf aktivieren")
Language:add(code, "Bots can spawn on vehicles inside C-130 gunship", "Bots können auf Fahrzeugen im C-130-Kampfhubschrauber erscheinen")
Language:add(code, "Bot Team Names", "Bot-Teamnamen")
Language:add(code, "If the bots shall have their names based on their team", "Wenn die Bots Namen basierend auf ihrem Team haben sollen")
Language:add(code, "The minimum time a bot shoots at one player", "Die Mindestzeit, die ein Bot auf einen Spieler schießt")
Language:add(code, "Explore", "Erkunden")
Language:add(code, "Draw Parsed Spawns", "Zeichnen Sie Parsen -Spawns")
Language:add(code, "Draw the parsed spawn-points with parsed links", "Zeichnen Sie die analysierten Spawn-Punkte mit analysierten Links")
Language:add(code, "AA Aim Worsening", "AA AIM -Verschlechterung")
Language:add(code, "Make bots in AA aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Machen Sie Bots im AA Zielen schlechter: Für Schwierigkeiten: 0 = kein Offset (hart), 1 oder sogar größer = mehr schwanken (einfach)")
Language:add(code, "Gunship Aim Worsening", "Verschlechterung des Waffenziels")
Language:add(code, "Make bots in Gunships aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Machen Sie Bots in Waffenzielezieleziele: Für Schwierigkeiten: 0 = kein Versatz (hart), 1 oder noch größer = mehr schwanken (einfach)")
Language:add(code, "Chopper Aim Worsening", "Hubschrauberziel verschlechtern sich")
Language:add(code, "Make bots in Choppers aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Bots in Choppern schlechter machen: Aus Schwierigkeiten: 0 = kein Offset (hart), 1 oder sogar größer = mehr schwanken (einfach)")
Language:add(code, "Vehicle Plane Aim Worsening", "Fahrzeugflugzeugzielverschlechterung")
Language:add(code, "See VehicleAimWorsening, only for Planes", "Siehe Fahrzeuge, nur für Flugzeuge")
Language:add(code, "Max Shoot Distance Air Missiles", "Max Schießstrecke Luftraketen")

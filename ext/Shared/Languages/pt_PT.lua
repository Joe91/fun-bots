...
Language:add(code, "Vehicle Aim Worsening", "Piora da mira do veículo")
Language:add(code, "Make bots in vehicles aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Faça bots em veículos apontarem pior: para dificuldade: 0 = sem deslocamento (difícil), 1 ou ainda maior = mais oscilação (fácil)")
Language:add(code, "Target distance waypoint air vehicles", "Veículos aéreos de waypoint de distância alvo")
Language:add(code, "The distance the bots have to reach to continue with the next Waypoint on air vehicles", "A distância que os bots devem alcançar para continuar com o próximo Waypoint em veículos aéreos")
Language:add(code, "Additional reaciton-time of bots", "Tempo de reação adicional dos bots")
Language:add(code, "Additional delay for bots, dependant of skill (might also be 0)", "Atraso adicional para bots, dependendo da habilidade (também pode ser 0)")
Language:add(code, "Point of Interst", "Ponto de Interesse")
Language:add(code, "POI", "PI")
Language:add(code, "Beacon", "Baliza")
Language:add(code, "Use jets", "Usar jatos")
Language:add(code, "Bots can use jets", "Bots podem usar jatos")
Language:add(code, "Bots Revive Bots", "Bots revivem bots")
Language:add(code, "Bots revive other Bots", "Bots revivem outros Bots")
Language:add(code, "Defend objectives", "Defender objetivos")
Language:add(code, "Bots will stay on captured objectives and defend them", "Os bots permanecerão nos objetivos capturados e os defenderão")
Language:add(code, "Enable vehicle paradrop", "Ativar paraquedas do veículo")
Language:add(code, "Bots can spawn on vehicles inside C-130 gunship", "Os bots podem aparecer em veículos dentro do caça C-130")
Language:add(code, "Bot Team Names", "Nomes de equipes de bots")
Language:add(code, "If the bots shall have their names based on their team", "Se os bots tiverem seus nomes baseados em sua equipe")
Language:add(code, "The minimum time a bot shoots at one player", "O tempo mínimo que um bot atira em um jogador")
Language:add(code, "Explore", "Explorar")
Language:add(code, "Draw Parsed Spawns", "Desenhe aparas analisadas")
Language:add(code, "Draw the parsed spawn-points with parsed links", "Desenhe os pontos de desova analisados ​​com links analisados")
Language:add(code, "AA Aim Worsening", "AA Aim piora")
Language:add(code, "Make bots in AA aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Faça os bots em AA mire pior: Para dificuldade: 0 = sem deslocamento (difícil), 1 ou ainda maior = mais oscilante (fácil)")
Language:add(code, "Gunship Aim Worsening", "O objetivo é pior")
Language:add(code, "Make bots in Gunships aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Faça os bots nos navios de guerra, mira pior: para dificuldade: 0 = sem deslocamento (difícil), 1 ou ainda maior = mais influência (fácil)")
Language:add(code, "Chopper Aim Worsening", "Piorando o objetivo do helicóptero")
Language:add(code, "Make bots in Choppers aim worse: for difficulty: 0 = no offset (hard), 1 or even greater = more sway (easy)", "Faça os bots nos helicópteros mirarem pior: para dificuldade: 0 = sem deslocamento (difícil), 1 ou ainda maior = mais oscilação (fácil)")
Language:add(code, "Vehicle Plane Aim Worsening", "Planejamento do objetivo do avião do veículo")
Language:add(code, "See VehicleAimWorsening, only for Planes", "Veja vehicleaimworsening, apenas para aviões")
Language:add(code, "Max Shoot Distance Air Missiles", "Mísseis de ar a distância máxima")
Language:add(code, "Meters before bots will start shooting with stingers", "Medidores antes de os bots começarem a atirar com Stingers")
Language:add(code, "Max Raycast Distance for Gunships", "MAX RAYCAST Distância para navios")
...
Language:add(code, "Follow Me", "Me siga")
Language:add(code, "Stop Following", "Pare de seguir")

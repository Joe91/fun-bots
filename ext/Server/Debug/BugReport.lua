...
-- Do not change this without input from @Firjens (unless minor or text changes) to prevent unwanted behaviour.
-- @author Firjen
...
-- Timestamp of the latest report created by anyone with permission. Used for reporting cooldown.
local DEBUG_LAST_REPORT = 0

-- Cooldown between bug reports in milliseconds. You are limited to a certain report per 24 hours, and it's useless to create a new report when no major changes were made.
...
function BugReport:GenerateReport(p_Player)
	...
	-- Contact the eligible end point to see if we are allowed to even make a bug report. We may be rate limited? We may be suspended because we abused it?
	...
	Net:GetHTTPAsync(s_CheckURL, function(p_HttpResponse)
		...
		-- Rate limiting, a limited amount of reports is allowed to be made within 24 hours.

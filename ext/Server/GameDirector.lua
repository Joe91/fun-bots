	...
	if Globals.IsConquest then -- valid for all Conquest-types
		...
		while s_Entity ~= nil do
			...
			if s_ObjectiveName ~= "" then
				local s_Objective = self:getObjectiveObject(s_ObjectiveName)
...
function GameDirector:OnMcomDestroyed(p_Player)
	...
	local s_SubObjective = self:getSubObjectiveFromObj(s_Objective)
	local s_TopObjective = self:getObjectiveFromSubObj(s_Objective)
...
function GameDirector:_updateValidObjectives()
	...
	if (self.m_McomCounter % 2) == 0 then
		...
		for _, l_Objective in pairs(self.m_AllObjectives) do
			...
			if not l_Objective.isSpawnPath and not l_Objective.isEnterVehiclePath then
...
function GameDirector:checkForExecution(p_Point, p_TeamId)
	...
	local s_Objective = self:getObjectiveObject(s_Mcom)
...
function GameDirector:_SetVehicleObjectiveState(p_Position, p_Value)
	...
	for _, l_Waypoints in pairs(s_Paths) do
		if l_Waypoints[1].Data.Objectives ~= nil and #l_Waypoints[1].Data.Objectives == 1 then
			local s_ObjectiveObject = self:getObjectiveObject(l_Waypoints[1].Data.Objectives[1])
		...
		for _, l_Path in pairs(s_PathsWithObjective) do
			if not s_PathsDone[l_Path] then
				...
				if s_Node ~= nil and s_Node.Data.Objectives ~= nil then
					if #s_Node.Data.Objectives == 1 then --possible path
...
function GameDirector:_translateObjective(p_Position, p_Name)
	if p_Name == nil or self.m_Translations[p_Name] == nil then
		local s_AllObjectives = m_NodeCollection:GetKnownOjectives()
		local s_PathsDone = {}
		...
		for l_Objective, l_Paths in pairs(s_AllObjectives) do
			for _, l_Path in pairs(l_Paths) do
				if not s_PathsDone[l_Path] then
					local s_Node = m_NodeCollection:Get(1, l_Path)
					if s_Node ~= nil and s_Node.Data.Objectives ~= nil then
						if #s_Node.Data.Objectives == 1 then --possible objective
							local s_Valid = true
							local s_TempObject = self:getObjectiveObject(l_Objective)
							if s_TempObject ~= nil and (s_TempObject.isSpawnPath or s_TempObject.isEnterVehiclePath) then -- or s_TempObject.isBase
								s_Valid = false
							end
							if s_Valid then
...
function GameDirector:isBasePath(p_ObjectiveNames)
	...
	for _, l_ObjectiveName in pairs(p_ObjectiveNames) do
		local s_Objective = self:getObjectiveObject(l_ObjectiveName)
...
function GameDirector:_useSubobjective(p_BotTeam, p_ObjectiveName)
	local s_Use = false
	local s_Objective = self:getObjectiveObject(p_ObjectiveName)
...
function GameDirector:OnPlayerEnteredCapturePoint(p_Player, p_CapturePoint)
	...
	local s_ObjectiveName = self:_translateObjective(p_CapturePoint.transform.trans, p_CapturePoint.name)
	self:_updateObjective(s_ObjectiveName, {
		isAttacked = p_CapturePoint.isAttacked  -- team = p_CapturePoint.team,
	})
end
...
function GameDirector:OnEngineUpdate(p_DeltaTime)
	...
	if self.m_UpdateLast >= self.m_UpdateInterval then
		...
		local s_MaxAssigns = {0, 0}
		for i = 1, 2 do
			if self.m_BotsByTeam[i] ~= nil then
				s_MaxAssigns[i] = math.floor(#self.m_BotsByTeam[i] / 2)
					...
					if l_Bot.m_Player.alive then
						...
						for _, l_Objective in pairs(self.m_AllObjectives) do
							if not l_Objective.subObjective then
								if not l_Objective.isBase and l_Objective.active and not l_Objective.destroyed then
						...
						if s_ClosestObjective ~= nil then
							local s_Objective = self:getObjectiveObject(s_ClosestObjective)
							l_Bot:setObjective(s_ClosestObjective)
							s_Objective.assigned[l_BotTeam] = s_Objective.assigned[l_BotTeam] + 1
						end
					end
				else
					local s_Objective = self:getObjectiveObject(l_Bot:getObjective())
					local s_ParentObjective = self:getObjectiveFromSubObj(s_Objective.name)
					s_Objective.assigned[l_BotTeam] = s_Objective.assigned[l_BotTeam] + 1
					if s_ParentObjective ~= nil then
						local s_TempObjective = self:getObjectiveObject(s_ParentObjective)
...
function GameDirector:useVehicle(p_BotName, p_Objective)
	local s_TempObjective = self:getObjectiveObject(p_Objective)

		...
		for i, l_Bot in pairs(self._BotsWithoutPath) do
			...
			if l_Bot.m_Player.soldier ~= nil then
				...
				if s_Node ~= nil then
					...
					s_SoldierKit, s_Appearance, s_SoldierCustomization = self:getKitApperanceCustomization(l_Bot.m_Player.teamId, l_Bot.m_Kit, l_Bot.m_Color, l_Bot.m_Primary, l_Bot.m_Pistol, l_Bot.m_Knife, l_Bot.m_PrimaryGadget, l_Bot.m_SecondaryGadget, l_Bot.m_Grenade)
...
function BotSpawner:OnRespawnBot(p_BotName)
	...
	if s_SpawnMode == 2 then --spawnInLine
		...
		self:spawnBot(s_Bot, s_Transform, false)
...
function BotSpawner:spawnBotRow(p_Player, p_Length, p_Spacing)
	for i = 1, p_Length do
		...
		if s_Name ~= nil then
			...
			self:spawnBot(s_Bot, s_Transform, true)
		end
	end
end
...
function BotSpawner:_spawnSigleWayBot(p_Player, p_UseRandomWay, p_ActiveWayIndex, p_IndexOnPath, p_ExistingBot, p_ForcedTeam)
	...
	if s_IsRespawn then
		...
		s_SquadId = self:getSquad(s_TeamId)
	end
	...
	if s_Name ~= nil or s_IsRespawn then
		...
		if s_IsRespawn then
			...
			self:spawnBot(p_ExistingBot, s_Transform, false)
		else
...
function BotSpawner:SelectLoadout(p_Bot, p_SetKit)
	...
	self:setBotWeapons(p_Bot, s_BotKit, s_WriteNewKit)
...
function BotSpawner:spawnBot(p_Bot, p_Trans, p_SetKit)
	...
	local s_SoldierKit = nil
	local s_Appearance = nil
	s_SoldierKit, s_Appearance, s_SoldierCustomization = self:getKitApperanceCustomization(p_Bot.m_Player.teamId, s_BotKit, s_BotColor, p_Bot.m_Primary, p_Bot.m_Pistol, p_Bot.m_Knife, p_Bot.m_PrimaryGadget, p_Bot.m_SecondaryGadget, p_Bot.m_Grenade)
